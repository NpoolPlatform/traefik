...
docker build --build-arg KUBE_VERSION=v0.20.2 --build-arg USER=$USER --build-arg UID=$MUID --build-arg GID=$MGID --build-arg ALL_PROXY=$all_proxy -f "./script/codegen.Dockerfile" \
             -t "${IMAGE_NAME}" \
             "."
